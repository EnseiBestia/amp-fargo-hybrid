...
// --------------------- Interface ExporterMojo ---------------------
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        if (getProject().getPackaging().equals("war") && getProject().hasParent()) {
            ...
            log("Assuming '" + moduleName + "' has hibernate.cfg.xml in its src/test/resources directory");
            getComponentProperties().put("configurationfile",
                    getProject().getBasedir() + "/" + pathToParent + moduleName + "/src/test/resources/hibernate.cfg.xml");
        }

        if (getComponentProperty("configurationfile") == null) {
            getComponentProperties().put("configurationfile", "src/test/resources/hibernate.cfg.xml");
        }
    ...
    private void addEntityToHibernateCfgXml(String hibernateCfgXml) throws MojoFailureException {
        ...
        try {
            FileUtils.writeStringToFile(new File(
                    getComponentProperty("configurationfile", "src/test/resources/hibernate.cfg.xml")), hibernateCfgXml);
            FileUtils.writeStringToFile(new File("src/main/resources/hibernate.cfg.xml"), hibernateCfgXml);            
        } catch (IOException io) {
            throw new MojoFailureException(io.getMessage());
        }
    }
